	...
	class functions{
		...
		function doPages($page_size, $thepage, $query_string, $total=0, $keyword) {
			...
			if($current==1) {
				...
				$i = $current-1;
				echo '<a href="'.$thepage.'?page='.$i.$query.'&keyword='.$keyword.'" rel="nofollow" title="go to page '.$i.'">Previous</a>&nbsp;';
				echo '<p>...</p>&nbsp;';
			}
			...
			if($current < $total_pages) {
				$i = $current+1;
				echo '<p>...</p>&nbsp;';
				echo '<a href="'.$thepage.'?page='.$i.$query.'&keyword='.$keyword.'" rel="nofollow" title="go to page '.$i.'">Next</a>&nbsp;';
			} else {
				echo '';
			}
